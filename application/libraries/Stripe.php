...
class Stripe {
	 ...
	 * @param  string        A free form reference for the charge
	 */
	public function charge_card( $amount, $card, $desc, $stripe_key_secret ) {
		$params = array(
			'amount' => $amount,
			'currency' => 'usd',
			'card' => $card,
		...
		return $this->_send_request( 'charges', $params, STRIPE_METHOD_POST, $stripe_key_secret );
	}
	 ...
	 */
	public function card_token_info( $token_id, $stripe_key_secret ) {
		return $this->_send_request( 'tokens/'.$token_id, array(), "get", $stripe_key_secret );
	}
	 ...
	 */
	private function _send_request( $url_segs, $params = array(), $http_method = 'get', $stripe_key_secret ) {
